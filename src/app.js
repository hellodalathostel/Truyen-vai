...
import { openDaoDien as moDaoDien } from "./ui/daoDien/index.js";
// Đợt 6d — bảng điều khiển: thân màn nằm ở `src/ui/bangDieuKhien/`.
...
// Bảng phụ thuộc của màn "Bảng điều khiển" (Đợt 6d): chỉ hàm CÒN LẠI của app.
const BANG_DIEU_KHIEN_DEPS = {
  layNguoiChoi, avatarHtml, avatarStack, threadCard, navQuayLai, anhHolder,
};

function renderDashboard(story) {
  return moBangDieuKhien(story, BANG_DIEU_KHIEN_DEPS);
}
...
// Bảng phụ thuộc của màn "Chế độ Đạo diễn" (Đợt 6d): chỉ hàm CÒN LẠI của app.
const DAO_DIEN_DEPS = {
  app, currentStory, currentConv, ddTongQuanHtml,
  ddOf, ddTruoc, ddLuu, luuTruyen, hoiXacNhan,
  openSuaDinhChinh, openTaoHuong, openSuaHuong,
};

function openDaoDien() {
  return moDaoDien(DAO_DIEN_DEPS);
}
...
// -------------------------------------------------------------- form hồ sơ
// Bảng phụ thuộc của màn "Sửa hồ sơ ngoại hình" (Đợt 6d): chỉ hàm CÒN LẠI của app.
const SUA_NGOAI_HINH_DEPS = {
  hoiXacNhan, render, thuNhoAnh,
};

function openSuaNgoaiHinh(id, onXong, opts = {}) {
  return moSuaNgoaiHinh(id, onXong, opts, SUA_NGOAI_HINH_DEPS);
}
