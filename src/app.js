...
async function openTaoAnh(opts = {}) {
  ...
  const chon = anhChon();
  const pcs = dsPhongCach();
  const kts = dsKichThuoc();

  const body = el("div", { class: "anh-modal" });
  body.innerHTML =
    // Khung xem trước KHÔNG ghép `dataUrl` vào chuỗi HTML: một bản ghi ảnh cũ nằm sẵn
    // trong IndexedDB cũng là dữ liệu không đáng tin. Ảnh (kể cả ảnh đang sửa) được dựng
    // bằng DOM ở `paintPreview()` ngay bên dưới.
  ...
  const actionsEl = el("div", { class: "row-gap anh-actions", "data-actions": "" });
  m.footEl.prepend(actionsEl);
  let url = suaAnh ? laDataUrlAnh(suaAnh.dataUrl) : "";
  let chuThich = (suaAnh && suaAnh.chuThich) || "";
  ...
  let promptDaDung = "";
  let busy = false;
  // Cổng người lớn cho ảnh (đọc trạng thái SỐNG mỗi lần gọi): truyện đang ở chế độ người
  // lớn mà trong cảnh có nhân vật CHƯA thoả `laNguoiLon()` ⇒ màn tạo ảnh bị CHẶN HẲN.
  // Không có nhánh "ép về chân dung an toàn" nữa: người dùng phải sửa xác nhận/tuổi của
  // nhân vật đó trước. Truyện KHÔNG ở chế độ người lớn ⇒ danh sách luôn rỗng (như cũ).
  const nvChuaXacNhan = () =>
    laCheDoNguoiLon(story) ? nvtsCoMat(story, conv).filter((c) => !laNguoiLon(c)) : [];
  const chanViChuaXacNhan = () => {
    const ds = nvChuaXacNhan();
    if (!ds.length) return false;
    const ten = ds.map((c) => c.ten).join(", ");
    setStatus(
  ...
  // Khung xem trước có sẵn ảnh đã lưu (kể cả ảnh đang sửa) thì dựng bằng DOM + `.src`,
  // sau khi đã lọc qua `laDataUrlAnh()`.
  if (url) paintPreview(url);
  ...
  dat("prompt", suaAnh ? tachNgoaiHinh(suaAnh.prompt) : "");
  dat("loaiTru", suaAnh ? suaAnh.loaiTru : chon.loaiTru);
  dat("ghiChu", "");
  {
    const pc = F("phongCach");
    if (pc) pc.value = (suaAnh && suaAnh.phongCach) || chon.phongCach || (pcs[0] && pcs[0].id) || "";
    if (pc && !pc.value) pc.selectedIndex = 0;
    const kt = F("kichThuoc");
    if (kt) kt.value = (suaAnh && suaAnh.kichThuoc) || chon.kichThuoc || (kts[0] && kts[0].id) || "";
    if (kt && !kt.value) kt.selectedIndex = 0;
  }

  // ---- Nhân vật trong khung hình: nhận diện tên → ghép ngoại hình từ thư viện ----
  // Người dùng là người quyết định: chip chỉ ĐỀ XUẤT. Một tên được nhắc trong prompt
  // không có nghĩa là người đó phải xuất hiện trong khung hình — bỏ chip là bỏ hẳn.
  const hoSoMap = hoSoTheoId(dsNgoaiHinh());
  const ungVien = ungVienNgoaiHinh(story, conv, dsNgoaiHinh());
  const pickEl = body.querySelector("[data-nh-pick]");
  const themIds = new Set(); // thêm tay (không nhận diện ra)
  const boQuaIds = new Set(); // người dùng đã bỏ chọn
  let tuDongIds = new Set(); // nhận diện từ prompt + nhân vật đang có mặt trong cảnh
  let trungTen = [];
  if (suaAnh && Array.isArray(suaAnh.hoSoIds)) for (const id of suaAnh.hoSoIds) if (hoSoMap[id]) themIds.add(id);

  // Hồ sơ sẽ được ghép: mọi thứ nhận diện/thêm tay, TRỪ những gì người dùng đã bỏ chọn.
  function dsChon() {
    const ra = [];
    const da = new Set();
    const day = (id) => {
    ...
    for (const u of ungVien) if (tuDongIds.has(u.hoSoId)) day(u.hoSoId);
    for (const h of dsNgoaiHinh()) if (tuDongIds.has(h.id)) day(h.id);
    for (const id of themIds) day(id);
    return ra;
  }

  function nhanDienLai() {
    const text = tachNgoaiHinh(F("prompt") ? F("prompt").value : "") + "\n" + tachNgoaiHinh(F("ghiChu") ? F("ghiChu").value : "");
    const kq = nhanDienNgoaiHinh(text, ungVien);
    trungTen = kq.trung;
    const moi = new Set(kq.chon);
    // Nhân vật đang CÓ MẶT trong cảnh mặc định được chọn: khung hình minh hoạ đúng cảnh
    // đang diễn ra nên họ gần như chắc chắn ở trong đó. Người dùng bỏ chip nếu không muốn.
    for (const c of nvtsCoMat(story, conv)) if (c.ngoaiHinhId && hoSoMap[c.ngoaiHinhId]) moi.add(c.ngoaiHinhId);
    // NGƯỜI CHƠI cũng ở trong mọi cảnh, nên hồ sơ của họ mặc định được chọn khi đã liên
    // kết — bỏ chip nếu khung hình không muốn thấy người chơi (góc nhìn thứ nhất…).
    const hNC = hoSoNguoiChoi(story, dsNgoaiHinh());
    if (hNC) moi.add(hNC.id);
    tuDongIds = moi;
  }
  ...
  function paintPick() {
    ...
    for (const u of ungVien) if (tuDongIds.has(u.hoSoId) || themIds.has(u.hoSoId) || boQuaIds.has(u.hoSoId)) them(u.hoSoId);
    for (const id of themIds) them(id);
    for (const t of trungTen) for (const id of t.ids) them(id); // trùng tên ⇒ hiện để người dùng tự chọn
    const chon = new Set(dsChon().map((h) => h.id));
    // Hồ sơ của NGƯỜI CHƠI được đánh dấu riêng: cùng một hồ sơ có thể vừa là của một
    // nhân vật vừa là của người chơi, nên nhãn này chỉ nói "hồ sơ này cũng là của bạn".
    const cuaNguoiChoi = new Set(ungVien.filter((u) => u.laNguoiChoi).map((u) => u.hoSoId));
    const chip = (id) => {
      const h = hoSoMap[id];
      const on = chon.has(id);
      return (
        '<button type="button" class="nh-chip' + (on ? " on" : "") + '" data-nh-chip="' + esc(id) + '" aria-pressed="' + (on ? "true" : "false") + '">' +
        '<span class="nh-chip-ten">' + esc(tenHoSo(h)) + "</span>" +
        (cuaNguoiChoi.has(id) ? '<span class="nh-chip-nguoi">bạn</span>' : "") +
    ...
    pickEl.innerHTML =
      ...
      (hienThi.length
        ? '<div class="nh-chips">' + hienThi.map(chip).join("") + "</div>"
  ...
  // Ô mô tả CHỈ chứa mô tả cảnh. Khối ngoại hình cố định KHÔNG bao giờ được ghi vào ô đó:
  // nó hiện riêng, chỉ đọc, và chỉ được ghép vào prompt đúng lúc dựng ảnh.
  //
  // Trước đây khối được nhúng thẳng vào ô rồi lần ghép sau cắt "từ nhãn tới hết" — nên chữ
  // người dùng viết thêm ở cuối ô (yêu cầu trang phục, góc nhìn…) bị xoá mất mỗi lần chip
  // đổi hoặc mô tả được viết lại. Giờ không còn đường nào ghi vào ô này ngoài người dùng.
  // Hồ sơ đã THỬ dịch nhưng vẫn chưa có bản tiếng Anh (máy chủ AI lỗi / không trả về). Khối
  // vẫn dùng đúng chữ người dùng gõ — nhưng phải NÓI RA, đừng để người dùng tưởng prompt đã
  // là tiếng Anh.
  const thieuDich = new Set();
  function veKhoiNgoaiHinh() {
    const box = body.querySelector("[data-nh-khoi]");
    if (!box) return;
    const ds = dsChon();
  ...
  // ---- Bản dịch tiếng Anh của ngoại hình ----
  ...
  let dangDich = false;
  const daThuDich = new Set(), dichHong = new Set();
  async function damBaoNgoaiHinhEn(thuLai) {
    if (dangDich) return false;
    if (thuLai) { daThuDich.clear(); dichHong.clear(); }
    const can = dsChon().filter((h) => canDichNgoaiHinh(h) && !daThuDich.has(h.id));
    if (!can.length) return false;
    dangDich = true;
    for (const h of can) daThuDich.add(h.id);
    try {
      ...
      for (const r of kq) {
        ...
        if (h.moTaEn || h.tranhEn) {
          coMoi = true;
          if (!(await luuBanDichNgoaiHinh(h))) dichHong.add(h.id);
        }
      }
      for (const h of can) {
        const ht = getNgoaiHinh(h.id) || h;
        if (canDichNgoaiHinh(ht) || dichHong.has(h.id)) thieuDich.add(h.id);
        else thieuDich.delete(h.id);
      }
      veKhoiNgoaiHinh();
      return coMoi;
    } catch (e) {
      console.error("[Truyện Vai] không dịch được ngoại hình:", e);
      for (const h of can) thieuDich.add(h.id);
      veKhoiNgoaiHinh();
      return false;
    } finally {
      dangDich = false;
  ...
  function capNhatKhoiNgoaiHinh() {
    ...
    if (dsChon().some((h) => canDichNgoaiHinh(h) && !daThuDich.has(h.id))) damBaoNgoaiHinhEn(false);
  }

  function moChonThem() {
    const daCo = new Set();
    for (const u of ungVien) if (tuDongIds.has(u.hoSoId) || themIds.has(u.hoSoId) || boQuaIds.has(u.hoSoId)) daCo.add(u.hoSoId);
    for (const id of themIds) daCo.add(id);
    const ds = dsNgoaiHinh().filter((h) => !daCo.has(h.id));
    if (!ds.length) {
    ...
    b.innerHTML =
      ...
      '<div class="nh-chips">' +
      ds.map((h) => '<button type="button" class="nh-chip" data-nh-add="' + esc(h.id) + '">' +
        '<span class="nh-chip-ten">' + esc(tenHoSo(h)) + "</span>" +
    ...
    b.addEventListener("click", (e) => {
      ...
      if (!t2) return;
      themIds.add(t2.dataset.nhAdd);
      boQuaIds.delete(t2.dataset.nhAdd);
      t2.remove();
      paintPick();
      capNhatKhoiNgoaiHinh();
      setStatus("Sẽ ghép ngoại hình của: " + dsChon().map(tenHoSo).join(", ") + ".");
    });
  }
  ...
  if (pickEl) {
    pickEl.addEventListener("click", (e) => {
      ...
      const dangChon = dsChon().some((h) => h.id === id);
      if (dangChon) {
        boQuaIds.add(id);
        themIds.delete(id);
      } else {
        boQuaIds.delete(id);
        if (!tuDongIds.has(id)) themIds.add(id);
      }
      nhanDienLai();
      paintPick();
      capNhatKhoiNgoaiHinh();
      const ds = dsChon();
      setStatus(ds.length ? "Sẽ ghép ngoại hình của: " + ds.map(tenHoSo).join(", ") + "." : "Không ghép hồ sơ ngoại hình nào vào khung hình này.");
    });
  }
  {
    // Gõ tay tên nhân vật vào ô mô tả ⇒ chip cập nhật NGAY, và khối ngoại hình của những
    // người vừa được nhận diện cũng được ghép vào (nếu khối đang khác). Không có bước này
    // thì người dùng gõ tên mà ngoại hình không bao giờ được áp — trái với mục tiêu.
    const nhanLai = debounce(() => { nhanDienLai(); paintPick(); capNhatKhoiNgoaiHinh(); }, 250);
  ...
  function paintPreview(u) {
    if (!previewEl) return;
    const ok = laDataUrlAnh(u);
    if (ok) {
      ganAnhVao(previewEl, ok, "", false);
    } else {
      previewEl.innerHTML = '<span class="anh-empty">Khung hình sẽ hiện ở đây</span>';
    }
  }
  function renderActions() {
    if (!actionsEl) return;
    const nut = (id, nhan, ic, cls, dis) =>
      '<button class="btn btn-sm' + (cls ? " " + cls : "") + '" data-act2="' + id + '"' + (dis ? " disabled" : "") + ">" + icon(ic, 14) + " " + esc(nhan) + "</button>";
    actionsEl.innerHTML =
      nut("viet-lai", "Viết lại", "sparkle", "", busy && !url) +
      (url
        ? nut("dung-lai", "Dựng lại", "refresh", "", busy) + nut("luu", "Đưa vào truyện", "check", "btn-primary", busy)
        : nut("dung", "Dựng khung hình", "image", "btn-primary", busy));
  }
  function setBusy(b) {
    busy = b;
  ...
  async function vietLai() {
    ...
    try {
      ...
      if (out.prompt) dat("prompt", tachNgoaiHinh(out.prompt));
      if (out.loaiTru) dat("loaiTru", out.loaiTru);
      if (out.chuThich) chuThich = out.chuThich;
      nhanDienLai();
      paintPick();
      capNhatKhoiNgoaiHinh();
      const dsHien = dsChon();
  ...
  async function dungAnh() {
    ...
    if (chanViChuaXacNhan()) return;
    // Ghép prompt NGAY TRƯỚC KHI DỰNG: lấy đúng chữ trong ô mô tả (kể cả phần người dùng
    // vừa viết thêm ở cuối) rồi nối khối ngoại hình của những hồ sơ đang chọn vào cuối.
    // KHÔNG ghi ngược prompt đã ghép vào ô — prompt đã ghép là dữ liệu DẪN XUẤT, chỉ đi
    // theo lệnh gửi máy vẽ và bản ghi ảnh.
    ...
    if (dsChon().some(canDichNgoaiHinh)) {
      setStatus("Đang dịch ngoại hình sang tiếng Anh…");
      await damBaoNgoaiHinhEn(true);
    }
    const chuaDich = dsChon().filter(canDichNgoaiHinh);
    if (chuaDich.length) console.warn("[Truyện Vai] chưa dịch được ngoại hình:", chuaDich.map((h) => h.id));
    const dsHoSo = dsChon();
    const gocPrompt = tachNgoaiHinh(F("prompt") ? F("prompt").value : "").trim();
    const prompt = ghepPromptNgoaiHinh(gocPrompt, dsHoSo);
    if (!prompt) {
      setStatus("Chưa có mô tả. Bấm “Viết lại mô tả”, hoặc tự viết một mô tả tiếng Anh.");
      return;
    }
    promptDaDung = prompt;
    const pc = F("phongCach") ? F("phongCach").value : "";
    const kt = F("kichThuoc") ? F("kichThuoc").value : "768x512";
    const ltGoc = F("loaiTru") ? F("loaiTru").value.trim() : "";
    ...
    // (Không còn nhánh "khung an toàn": trường hợp nhân vật chưa xác nhận đã bị chặn ở trên.)
    const lt = gopLoaiTruNgoaiHinh(ltGoc, dsHoSo);
    const c = anhChon();
    c.phongCach = pc;
    c.kichThuoc = kt;
    luuAnhChon();
    ...
    try {
      const them = themPhongCach(pc);
      const raw = await AI.taoAnh({ prompt: prompt + (them ? ", " + them : ""), loaiTru: lt, kichThuoc: kt });
      url = await nenAnhDataUrl(raw, kt === "512x512" ? 512 : 768, 0.9);
      paintPreview(url);
      setStatus("Xong. “Đưa vào hội thoại” để chèn vào mạch truyện, hoặc “Dựng lại” nếu chưa vừa ý.");
    } catch (e) {
      paintPreview("");
      setStatus("Lỗi: " + (e.message || e));
    }
  ...
  async function luuVaoHoiThoai() {
    if (busy || !url) return;
    setBusy(true);
    const anhTruoc = anhCua(story).slice();
    const tnTruoc = getMessages(conv.id).slice();
    const suaCu = suaMsg ? { anhId: suaMsg.anhId, chuThich: suaMsg.chuThich } : null;
    try {
      const meta = newAnh({
        ...
        prompt: promptDaDung || (F("prompt") ? ghepPromptNgoaiHinh(F("prompt").value, dsChon()).trim() : ""),
        loaiTru: F("loaiTru") ? F("loaiTru").value.trim() : "",
        phongCach: F("phongCach") ? F("phongCach").value : "",
        kichThuoc: F("kichThuoc") ? F("kichThuoc").value : "",
        chuThich: chuThich || "",
        convId: conv.id,
        // Hồ sơ ngoại hình đã dùng cho khung hình: đi theo bản ghi ảnh để xuất riêng
        // một truyện kèm đúng những hồ sơ được tham chiếu.
        hoSoIds: dsChon().map((h) => h.id),
      });
      await luuAnh(story, meta);
      if (suaMsg) {
        const cu = suaMsg.anhId;
        const cuChu = suaMsg.chuThich;
        suaMsg.anhId = meta.id;
        suaMsg.chuThich = meta.chuThich;
        const ok = await luuTinNhan(conv.id, () => {
          suaMsg.anhId = cu;
          suaMsg.chuThich = cuChu;
        });
        if (!ok) {
          setBusy(false);
          return;
        }
        capNhatConv(conv, getMessages(conv.id));
        if (cu && cu !== meta.id) {
          try { await xoaAnh(story, cu); } catch (e) { baoLoiLuu(e, "ảnh cũ"); }
        }
      } else if (opts.sauMsgId) {
        const msg = makeMessage("anh", "🖼️ Ảnh cảnh", { anhId: meta.id, chuThich: meta.chuThich });
        const arr = getMessages(conv.id).slice();
        const idx = arr.findIndex((x) => x.id === opts.sauMsgId);
        if (idx >= 0) arr.splice(idx + 1, 0, msg); else arr.push(msg);
        try {
          await replaceMessages(conv.id, arr);
        } catch (e) {
          baoLoiLuu(e, "tin nhắn");
          setBusy(false);
          return;
      ...
      const daLuu = await luuTruyen(story);
      if (!daLuu) {
        // Ghi hỏng ⇒ KHÔNG đóng bảng và KHÔNG báo "đã thêm": trả tin nhắn + chỉ mục ảnh
        // về nguyên trạng, dọn bản ghi ảnh vừa ghi (nếu không sẽ thành ảnh mồ côi), và
        // để người dùng bấm lại — ảnh vẫn còn trên bảng.
        story.anh = anhTruoc;
        store.messagesCache[conv.id] = tnTruoc;
        if (suaCu && suaMsg) {
          suaMsg.anhId = suaCu.anhId;
          suaMsg.chuThich = suaCu.chuThich;
        }
        capNhatConv(conv, tnTruoc);
        if (!suaAnh) {
          try { await xoaAnh(story, meta.id); } catch (e) { console.error(e); }
        }
        setStatus("Chưa ghi được xuống máy nên khung hình chưa vào truyện. Bấm “Đưa vào truyện” để thử lại.");
        setBusy(false);
        return;
      }
      m.close();
      render();
      toast(suaMsg ? "Đã thay khung hình." : "Đã thêm khung hình vào hội thoại.");
    } catch (e) {
      setStatus("Lỗi: " + (e.message || e));
      setBusy(false);
    }
  ...
  actionsEl.addEventListener("click", (e) => {
    ...
    if (a === "viet-lai") vietLai();
    else if (a === "dung" || a === "dung-lai") dungAnh();
    else if (a === "luu") luuVaoHoiThoai();
  });

  if (suaAnh) {
    paintPreview(url);
    setStatus("Sửa mô tả rồi bấm “Dựng lại”, hoặc “Đưa vào hội thoại” để dùng lại ảnh này.");
    renderActions();
  } else {
    paintPreview("");
    renderActions();
...
function openNewStoryModal() {
  const modes = (R.MauCotTruyen && R.MauCotTruyen()) || [];
  const theLoais = (R.TheLoai && R.TheLoai()) || [];
  ...
  const htmlWizard =
    ...
    '<div class="wizard-block"><div class="wizard-label">Bước 4 · Giao kèo (không bắt buộc)</div>' +
      htmlGiaoKeo(giaoKeoMacDinh(), "wiz") +
    "</div>";
  ...
  body.innerHTML =
    ...
    "</div>" +
    '<div data-pane="nhanh">' + htmlNhanh + "</div>" +
    '<div data-pane="nangcao" hidden>' + htmlWizard + "</div>";
  ...
  const m = modal({
    ...
    actions: [
      { label: "Huỷ", onClick: (mm) => mm.close() },
      { label: "Tạo cốt truyện", primary: true, onClick: (mm) => (body.dataset.cachTao === "nhanh" ? taoTruyenNhanh(mm) : taoTruyen(mm)) },
    ],
  });
  ...
  let mode = modes.length ? modes[0].id : "chuong";

  ganSuKienGiaoKeo(body, "wiz");

  const chonTheLoai = body.querySelector('[data-f="theLoai"]');
  if (chonTheLoai) {
    chonTheLoai.addEventListener("change", () => {
      const tl = theLoais.find((t) => t.id === chonTheLoai.value);
      if (!tl || !tl.bdsm) return;
      const bat = body.querySelector('[data-f="wizbat"]');
      if (bat && !bat.checked) {
        bat.checked = true;
        const fields = body.querySelector("[data-gk-fields]");
        if (fields) fields.hidden = false;
      }
      const kk = body.querySelector('[data-f="wizkhongKhi"]');
      if (kk && !kk.value.trim() && tl.khongKhi) kk.value = tl.khongKhi;
      const kq = body.querySelector('[data-f="wizkieuQuanHe"]');
      if (kq && !kq.value && tl.kieuQuanHe) {
        kq.value = tl.kieuQuanHe;
        paintGiaoKeo(body, "wiz");
      }
    });
  }

  const wizStub = () => ({
    ten: val("ten") || "Cốt truyện mới",
    boiCanh: val("boiCanh"),
    theLoaiTen: (theLoais.find((t) => t.id === val("theLoai")) || {}).ten || "",
    nhanVats: [],
    bienNienSu: [],
    giaoKeo: docGiaoKeo(body, "wiz", giaoKeoMacDinh()),
  });
  const wizBdsm = () =>
    !!(theLoais.find((t) => t.id === val("theLoai")) || {}).bdsm ||
    !!((body.querySelector('[data-f="wizbat"]') || {}).checked);
  ...
  body.addEventListener("click", async (e) => {
    ...
    if (optChon) {
      await chonHuongNhanVat({
        ...
        status,
        onXong: async (c) => {
          body.dataset.pendingCharacter = JSON.stringify(c);
    ...
    if (act.dataset.act === "wiz-suggest") {
      const tl = theLoais.find((t) => t.id === val("theLoai"));
      status.textContent = "Đang nhờ AI viết bối cảnh…";
      try {
        const seed = await AI.generateStorySeed({ theLoai: tl, yTuong: val("moTa"), giaoKeo: docGiaoKeo(body, "wiz", giaoKeoMacDinh()) });
        setVal("boiCanh", seed.boiCanh + (seed.luat ? "\n\nLuật thế giới:\n" + seed.luat : ""));
        if (!val("ten")) {
          const ten = (seed.boiCanh.split(/[.!?\n]/)[0] || "").trim();
          if (ten) setVal("ten", ten.slice(0, 60));
        }
    ...
    if (act.dataset.act === "wiz-character") {
      await sinhHuongNhanVat({ story: wizStub(), yTuong: val("moTa"), bdsm: wizBdsm(), scope: body, status });
      return;
    }
  });

  async function taoTruyen(mm) {
    const ten = val("ten");
    if (!ten) {
    ...
    const tl = theLoais.find((t) => t.id === val("theLoai"));
    let nhanVats = [];
    if (body.dataset.pendingCharacter) {
      try {
        const c = JSON.parse(body.dataset.pendingCharacter);
        nhanVats.push(newCharacter(Object.assign({}, c, { mau: MAU_CHOICES[Math.floor(Math.random() * MAU_CHOICES.length)] })));
      } catch (e) {}
    }
    const giaoKeo = docGiaoKeo(body, "wiz", giaoKeoMacDinh());
    ...
    if (giaoKeo.bat) {
      const xac = await xacNhan18PlusTruyen("Bạn đang bật giao kèo BDSM cho truyện này.", nhanVats);
      if (!xac.dongY) {
        giaoKeo.bat = false;
        toast("Chưa xác nhận 18+ cho đúng danh sách nhân vật nên truyện được tạo KHÔNG kèm lớp giao kèo.", "error");
      } else xacNhanMoiNguoiLon({ nhanVats });
    }
    const story = await createStory({
      ten, moTa: val("moTa"), theLoaiId: val("theLoai"), theLoaiTen: tl ? tl.emoji + " " + tl.ten : "",
      boiCanh: val("boiCanh"), mode, nguoiChoiTen: val("nguoiChoiTen") || "Bạn", nguoiChoiMoTa: val("nguoiChoiMoTa"),
      emoji: tl ? tl.emoji : "✦", nhanVats, giaoKeo,
      mucTieuChuong1: tl && tl.goiY ? "" : "",
    });
    mm.close();
    toast("Đã tạo cốt truyện “" + ten + "”.");
    await openStory(story.id, null);
  }
  ...
  const qcInp = (nhan, f, v) =>
    '<label class="field-label">' + esc(nhan) + '</label><input class="input" data-qc-f="' + f + '" value="' + esc(v || "") + '">';
  const qcArea = (nhan, f, v, rows) =>
    '<label class="field-label">' + esc(nhan) + '</label><textarea class="input" data-qc-f="' + f + '" rows="' + (rows || 3) + '">' + esc(v || "") + "</textarea>";

  function paintQcKetQua() {
    const box = body.querySelector("[data-qc-kq]");
    const q = body.__qcKq;
    if (!box) return;
    if (!q) {
      box.innerHTML = "";
  ...
  function docQcKetQua() {
    const q = body.__qcKq || {};
    const nhanVats = (q.nhanVats || [])
      .map((c, i) => {
        const card = body.querySelector('[data-qc-nv="' + i + '"]');
        if (!card) return c;
        const g = (f) => {
    ...
    return Object.assign({}, q, {
      ten: qcVal("ten") || q.ten,
      moTa: qcVal("moTa"),
      boiCanh: qcVal("boiCanh"),
      luat: qcVal("luat"),
      nguoiChoiTen: qcVal("nguoiChoiTen") || "Bạn",
      nguoiChoiMoTa: qcVal("nguoiChoiMoTa"),
      mucTieu: qcVal("mucTieu"),
      nhanVats,
    });
  }

  async function dungBanNhap() {
    const st = body.querySelector("[data-qc-status]");
    const nut = body.querySelector('[data-act="qc-dung"]');
    const tl = theLoais.find((t) => t.id === val("qcTheLoai"));
    const doiBdsm = !!((body.querySelector('[data-f="qcbdsm"]') || {}).checked) || !!(tl && tl.bdsm);
    // Tạo nhanh không được là đường vòng qua cửa 18+: chọn thể loại BDSM hay tích ô
    // "giao kèo" đều phải qua hộp xác nhận trước khi AI dựng bản nháp có lớp BDSM.
    let dungBdsm = doiBdsm;
    let ghiChu18 = "";
    if (doiBdsm && !body.__qc18) {
      const dongY = await xacNhan18Plus("Bản nháp bạn sắp dựng có giao kèo BDSM.");
      body.__qc18 = dongY;
      body.__qc18Ds = ""; // bản nháp chưa biết tên nhân vật ⇒ lúc TẠO sẽ liệt kê lại
      if (!dongY) {
        dungBdsm = false;
        const cb = body.querySelector('[data-f="qcbdsm"]');
        if (cb) cb.checked = false;
        ghiChu18 = "Chưa xác nhận 18+ nên bản nháp được dựng KHÔNG kèm lớp BDSM.";
      }
    }
    if (!doiBdsm) body.__qc18 = false;
    ...
    try {
      const kq = await AI.generateQuickStory({
        yTuong: val("qcYTuong"),
        theLoai: tl,
        soNhanVat: Number(val("qcSoNv")) || 3,
        bdsm: dungBdsm,
        giaoKeo: dungBdsm
          ? Object.assign(giaoKeoMacDinh(), { bat: true, nguoiLon: true, khongKhi: (tl && tl.khongKhi) || "" })
          : null,
      });
      body.__qcKq = kq;
      paintQcKetQua();
      const kqBox = body.querySelector("[data-qc-kq]");
  ...
  async function taoTruyenNhanh(mm) {
    if (!body.__qcKq) {
    ...
    const tl = theLoais.find((t) => t.id === val("qcTheLoai"));
    // Cửa 18+ lần nữa, ở đúng bước TẠO: dù bản nháp đã được dựng, người dùng có thể đã
    // tích ô "giao kèo" sau đó. Không có xác nhận thì lớp BDSM không được ghi vào truyện.
    const doiBdsm = !!((body.querySelector('[data-f="qcbdsm"]') || {}).checked) || !!(tl && tl.bdsm);
    let dongY18 = !!body.__qc18;
    if (doiBdsm && !dongY18) {
      dongY18 = await xacNhan18Plus("Truyện bạn sắp tạo sẽ bật giao kèo BDSM.");
      body.__qc18 = dongY18;
    }
    ...
    const batGk = doiBdsm && dongY18;
    const giaoKeo = Object.assign(
      giaoKeoMacDinh(),
      batGk ? { bat: true, nguoiLon: true, khongKhi: (tl && tl.khongKhi) || "", kieuQuanHe: (tl && tl.kieuQuanHe) || "" } : {}
    );
    const nhanVats = (q.nhanVats || []).map((c) =>
      newCharacter(Object.assign({}, c, { mau: MAU_CHOICES[Math.floor(Math.random() * MAU_CHOICES.length)] }))
    );
    // Người dùng vừa xác nhận 18+ cho cả truyện ⇒ ghi cờ tường minh cho từng nhân vật
    // TRƯỚC khi mở cổng — nhưng phải liệt kê ĐÚNG danh sách sẽ được ghi (xác nhận ở bước
    // dựng bản nháp chưa biết tên nhân vật, nên lúc TẠO phải hỏi lại nếu danh sách khác).
    if (giaoKeo.bat) {
      const dsTen = nhanVats.map((c) => c.ten).join("|");
      if (!dongY18 || body.__qc18Ds !== dsTen) {
        const xac = await xacNhan18PlusTruyen("Truyện bạn sắp tạo sẽ bật giao kèo BDSM.", nhanVats);
        if (!xac.dongY) {
          giaoKeo.bat = false;
          toast("Chưa xác nhận 18+ cho đúng danh sách nhân vật nên truyện được tạo KHÔNG kèm lớp giao kèo.", "error");
        } else {
          ...
          body.__qc18 = true;
          body.__qc18Ds = dsTen;
          xacNhanMoiNguoiLon({ nhanVats: nhanVats });
        }
      } else xacNhanMoiNguoiLon({ nhanVats: nhanVats });
    }
    const lyDoKhoa = chanNoiDungNguoiLon({ nhanVats: nhanVats });
    if (lyDoKhoa) {
      giaoKeo.bat = false;
    ...
    try {
      story = await createStory({
        ...
        moTa: q.moTa,
        theLoaiId: tl ? tl.id : "",
        theLoaiTen: tl ? tl.emoji + " " + tl.ten : "",
        boiCanh: q.boiCanh + (q.luat ? "\n\nLuật thế giới:\n" + q.luat : ""),
        mode: "chuong",
        nguoiChoiTen: q.nguoiChoiTen || "Bạn",
...
function openCharacterEditor(charId, opts = {}) {
  ...
  if (!story) return;
  const isNew = !charId;
  const c = isNew ? newCharacter({ mau: MAU_CHOICES[Math.floor(Math.random() * MAU_CHOICES.length)] }) : charById(story, charId);
  if (!c) return;
  // BẢN NHÁP của form. Nhân vật trong truyện KHÔNG bị sửa cho tới khi lưu thành công —
  // kể cả khi đổi liên kết hồ sơ ngoại hình. Không có bản nháp thì bấm Huỷ vẫn làm đổi
  // `nhanVats[].ngoaiHinhId`, và lần lưu truyện sau đó (một thao tác khác) sẽ ghi luôn
  // thay đổi mà người dùng tưởng đã huỷ.
  const nhap = JSON.parse(JSON.stringify(c));
  const mauList = MAU_CHOICES;
  ...
  const gkNoiDung =
    ...
    "</details>";
  const gkKhoi = gkBat
    ? '<div class="gk-char"><div class="gk-char-head">' + esc("⛓️ Vai trong giao kèo (BDSM M/M)") + "</div>" + gkNoiDung + "</div>"
  ...
  const avatarPreview = () => avatarHtml(story, nhap, 72);
  body.innerHTML =
    '<div class="char-editor-top">' +
      '<div class="char-preview" data-preview>' + avatarPreview() + "</div>" +
  ...
  const m = modal({
    ...
    body,
    actions: [
      ...(isNew ? [] : [{ label: "Xoá", danger: true, onClick: (mm) => { mm.close(); xoaNhanVat(c.id); } }]),
      { label: "Huỷ", onClick: (mm) => mm.close() },
      { label: isNew ? "Thêm nhân vật" : "Lưu", primary: true, onClick: async (mm) => { if (await luuNhanVat(nhap, body, isNew)) mm.close(); } },
    ],
  });

  const F = (f) => body.querySelector('[data-f="' + f + '"]');
  F("ten").value = nhap.ten === "Nhân vật mới" ? "" : nhap.ten;
  ...
  if (F("nguoiLon")) F("nguoiLon").checked = laNguoiLon(nhap);
  {
    ...
    const dongBoTuoi = () => {
      if (!nlEl) return;
      const tuoi = tuoiSo({ tuoi: tuoiEl ? tuoiEl.value : "" });
      const tre = tuoi !== null && tuoi < 18;
      if (tre) nlEl.checked = false;
      nlEl.disabled = tre;
      nlEl.title = tre ? "Nhân vật ghi tuổi dưới 18 — không thể đánh dấu là người trưởng thành." : "";
    };
  ...
  {
    if (F("vaiBdsm")) F("vaiBdsm").value = nhap.vaiBdsm || "";
    if (F("danhXung")) F("danhXung").value = nhap.danhXung || "";
    if (F("kinhNghiem")) F("kinhNghiem").value = nhap.kinhNghiem || "";
    if (F("phongCach")) F("phongCach").value = nhap.phongCach || "";
    if (F("khauVi")) F("khauVi").value = nhap.khauVi || "";
    if (F("gioiHan")) F("gioiHan").value = nhap.gioiHan || "";
    if (F("gioiHanCung")) F("gioiHanCung").value = nhap.gioiHanCung || "";
    if (F("luatRieng")) F("luatRieng").value = nhap.luatRieng || "";
    if (F("chamSocSau")) F("chamSocSau").value = nhap.chamSocSau || "";
    if (F("tinHieuRieng")) F("tinHieuRieng").value = nhap.tinHieuRieng || "";
    paintNvThich(body);
  }
  {
    const sl = body.querySelector("[data-suggest]");
    if (sl) {
      const maus = (R.MauNhanVat && R.MauNhanVat()) || [];
  ...
  function paintAvatar() {
    const p = body.querySelector("[data-preview]");
    if (p) p.innerHTML = avatarHtml(story, nhap, 72);
    const seg = body.querySelectorAll(".seg-btn");
    seg.forEach((b) => b.classList.toggle("on", b.dataset.av === nhap.avatarStyle));
    const extra = body.querySelector("[data-av-extra]");
    if (!extra) return;
  ...
  function dienNhanVat(gen) {
    ...
    if (Array.isArray(gen.soThich) && gen.soThich.length) {
      const soThichEl = body.querySelector('[data-f="soThich"]');
  ...
  body.addEventListener("click", async (e) => {
    ...
    const batGk = e.target.closest('[data-act="enable-giao-keo"]');
    if (batGk) {
      // Bật giao kèo = bật lớp BDSM: phải qua hộp xác nhận 18+ liệt kê ĐÚNG danh sách
      // nhân vật sẽ được ghi cờ, và phải kiểm tra tuổi của nhân vật trong truyện — không
      // thể "bật nhanh" bằng một cú bấm.
      const xac = await xacNhan18PlusTruyen("Bạn đang bật giao kèo BDSM cho truyện này.", story.nhanVats || []);
      if (!xac.dongY) {
        toast("Chưa xác nhận 18+ nên giao kèo chưa được bật.", "error");
        return;
      }
      // Người dùng vừa xác nhận 18+ cho cả truyện ⇒ ghi cờ tường minh cho từng nhân vật
      // TRƯỚC khi mở cổng, nếu không cổng sẽ chặn đúng cái nút vừa xác nhận.
      xacNhanMoiNguoiLon(story);
      const lyDo = chanNoiDungNguoiLon(story);
      if (lyDo) {
        toast(lyDo, "error");
        return;
      ...
      const d = body.querySelector("[data-gk-off]");
      if (d) {
        d.open = true;
    ...
    if (nvThich) {
      const hidden = body.querySelector('[data-f="soThich"]');
      if (hidden) {
        const ds = String(hidden.value || "").split(",").filter(Boolean);
        const id = nvThich.dataset.nvThich;
        const i = ds.indexOf(id);
        if (i >= 0) ds.splice(i, 1);
        else ds.push(id);
        hidden.value = ds.join(",");
        nvThich.classList.toggle("on", i < 0);
      }
      return;
    }
    ...
    if (mauNv) {
      ...
      if (t2 && t2.vaiBdsm && !giaoKeoOf(story).bat) {
        const d0 = body.querySelector("[data-gk-off]");
        if (d0) d0.open = true;
      }
      if (t2) {
        if (F("vaiTro") && t2.vaiTro) F("vaiTro").value = t2.vaiTro;
        if (F("tinhCach") && t2.tinhCach) F("tinhCach").value = t2.tinhCach;
        if (F("cachNoi") && t2.cachNoi) F("cachNoi").value = t2.cachNoi;
        if (F("moTa") && t2.moTa) F("moTa").value = t2.moTa;
        if (F("vaiBdsm") && t2.vaiBdsm) F("vaiBdsm").value = t2.vaiBdsm;
        if (F("danhXung") && t2.danhXung) F("danhXung").value = t2.danhXung;
        if (F("kinhNghiem") && t2.kinhNghiem) F("kinhNghiem").value = t2.kinhNghiem;
        if (F("phongCach") && t2.phongCach) F("phongCach").value = t2.phongCach;
        if (F("khauVi") && t2.khauVi) F("khauVi").value = t2.khauVi;
        if (F("gioiHan") && t2.gioiHan) F("gioiHan").value = t2.gioiHan;
        if (F("gioiHanCung") && t2.gioiHanCung) F("gioiHanCung").value = t2.gioiHanCung;
        if (F("luatRieng") && t2.luatRieng) F("luatRieng").value = t2.luatRieng;
        if (F("chamSocSau") && t2.chamSocSau) F("chamSocSau").value = t2.chamSocSau;
        if (F("tinHieuRieng") && t2.tinHieuRieng) F("tinHieuRieng").value = t2.tinHieuRieng;
        const soThichEl = body.querySelector('[data-f="soThich"]');
        ...
        if (sl) {
          ...
          note.textContent = "Đã điền mẫu “" + t2.ten + "”. Đặt tên và sửa lại theo ý bạn." +
            (t2.vaiBdsm && !giaoKeoOf(story).bat ? " Truyện chưa bật giao kèo — bấm “Bật giao kèo cho truyện này” ở phần bên trên để AI dùng hết phần BDSM." : "");
          sl.prepend(note);
        }
      }
      return;
    ...
    if (seg) {
      nhap.avatarStyle = seg.dataset.av;
      paintAvatar();
      return;
    }
    const dot = e.target.closest("[data-mau]");
    if (dot) {
      nhap.mau = dot.dataset.mau;
      paintAvatar();
      return;
    }
    const em = e.target.closest("[data-emoji]");
    if (em) {
      nhap.emoji = em.dataset.emoji;
      paintAvatar();
      return;
    }
    const optChon = e.target.closest("[data-nv-opt-chon]");
    if (optChon) {
      const khoiGk0 = body.querySelector("[data-gk-off]");
      await chonHuongNhanVat({
        scope: body,
        i: Number(optChon.dataset.nvOptChon),
        story,
        bdsm: gkBat || !!(khoiGk0 && khoiGk0.open),
        yTuongGoc: F("nvYeuCau").value.trim() || F("ten").value.trim() || F("moTa").value.trim(),
        status,
        onXong: async (gen) => { dienNhanVat(gen); },
      });
      return;
    }
    const act = e.target.closest("[data-act]");
    if (!act) return;
    if (act.dataset.act === "open-ngoai-hinh") {
      // Thư viện mở đè lên editor; đóng lại thì dựng lại danh sách liên kết.
      openNgoaiHinh(lamMoiLienKet);
      return;
    }
    if (act.dataset.act === "edit-ngoai-hinh") {
      openSuaNgoaiHinh(act.dataset.id, lamMoiLienKet);
      return;
    }
    if (act.dataset.act === "dong-ten-ngoai-hinh") {
      // Đồng bộ tên nhân vật theo TÊN CHÍNH của hồ sơ — chỉ điền vào form, người dùng vẫn
      // phải bấm Lưu; tin nhắn cũ không bị sửa (app không viết lại lịch sử).
      const sel = F("ngoaiHinhId");
      const h2 = sel && sel.value ? getNgoaiHinh(sel.value) : null;
      if (!h2 || !h2.tenChinh) return;
      F("ten").value = h2.tenChinh;
      dienLienKetNgoaiHinh(body, nhap);
      toast("Đã điền tên theo hồ sơ — bấm Lưu để áp dụng.");
      return;
    }
    if (act.dataset.act === "char-upload") {
      ...
      inp.onchange = async () => {
        const f = inp.files[0];
        if (!f) return;
        const dataUrl = await thuNhoAnh(f, 256);
        nhap.anh = dataUrl;
        nhap.avatarStyle = "anh";
        paintAvatar();
      };
      inp.click();
    }
    if (act.dataset.act === "char-ai") {
      const khoiGk = body.querySelector("[data-gk-off]");
      await sinhHuongNhanVat({
        story,
        yTuong: F("nvYeuCau").value.trim() || F("ten").value.trim() || F("moTa").value.trim(),
        bdsm: gkBat || !!(khoiGk && khoiGk.open),
        scope: body,
        status,
      });
      return;
    }
    if (act.dataset.act === "char-avatar-ai") {
      status.textContent = "Đang vẽ ảnh đại diện…";
      try {
        const promptText =
          "Chân dung nhân vật " + (F("ten").value || "nhân vật") + " trong một câu chuyện " + (story.theLoaiTen || "viễn tưởng") +
          ". Ngoại hình: " + (F("moTa").value || "bí ẩn") + ". Chất lượng cao, ánh sáng điện ảnh, nền đơn giản, cận cảnh khuôn mặt.";
        const dataUrl = await AI.generateAvatar(promptText);
        nhap.anh = dataUrl;
        nhap.avatarStyle = "anh";
        paintAvatar();
  ...
  // Dựng lại riêng khối liên kết (sau khi thêm/sửa hồ sơ trong thư viện) rồi gắn lại
  // sự kiện — danh sách <option> mới luôn khớp thư viện hiện tại.
  function lamMoiLienKet() {
    const cu = body.querySelector(".nh-link");
    ...
    const tmp = el("div");
    tmp.innerHTML = htmlLienKetNgoaiHinh(nhap);
    if (cu) cu.replaceWith(tmp.firstElementChild);
    else body.insertBefore(tmp.firstElementChild, body.querySelector('[data-f="tinhCach"]'));
    const bdMoi = body.querySelector('[data-f="bietDanh"]');
  ...
  // Tuổi của một người phải NHẤT QUÁN: nếu hồ sơ đã khai tuổi thì khi lưu, tuổi nhân
  // vật lấy theo hồ sơ — ảnh (dù trông trưởng thành) không bao giờ thay cho tuổi khai
  // báo. Đây là lý do khối này nằm ngay trong form, nói trước khi người dùng bấm Lưu.
  function capNhatGoiYTuoi() {
    const hint = body.querySelector("[data-nh-link-hint]");
    if (!hint) return;
    const sel = body.querySelector('[data-f="ngoaiHinhId"]');
    const h = sel && sel.value ? getNgoaiHinh(sel.value) : null;
    const tuoi = h ? tuoiSo({ tuoi: h.tuoi }) : null;
    if (h && tuoi !== null) {
      const hienTai = tuoiSo({ tuoi: F("tuoi") ? F("tuoi").value : "" });
      hint.textContent =
        "Hồ sơ “" + tenHoSo(h) + "” khai " + tuoi + " tuổi — khi lưu, tuổi nhân vật sẽ lấy theo hồ sơ" +
        (hienTai !== null && hienTai !== tuoi ? " (hiện đang ghi " + hienTai + ")" : "") +
        ". Tuổi khai báo là thứ quyết định, ảnh không thay thế được." +
        (tuoi < 18 ? " Hồ sơ ghi tuổi dưới 18 nên nhân vật KHÔNG thể là người trưởng thành." : "");
      return;
    }
    hint.textContent = dsNgoaiHinh().length
  ...
  // `nv` = BẢN NHÁP của form. Nhân vật trong truyện (`c`) KHÔNG bị đụng tới cho tới đây:
  // bấm Huỷ thì không có thay đổi nào cả (kể cả đổi liên kết hồ sơ ngoại hình), và nếu
  // ghi hỏng thì `c` được trả về nguyên trạng.
  async function luuNhanVat(nv, bodyEl, isNew2) {
    nv.ten = F("ten").value.trim() || "Nhân vật mới";
    ...
    {
      const tuoi = tuoiSo(nv);
      nv.nguoiLon = tuoi !== null && tuoi < 18 ? false : !!nv.nguoiLon;
    }
    ...
    // sơ (ảnh trông trưởng thành không thay cho tuổi khai báo). Áp lại cửa 18+ ngay.
    {
      ...
      if (hSo && tuoiSo({ tuoi: hSo.tuoi }) !== null) {
        nv.tuoi = String(hSo.tuoi);
        if (F("tuoi")) F("tuoi").value = nv.tuoi;
        const tuoi = tuoiSo(nv);
        if (tuoi !== null && tuoi < 18) nv.nguoiLon = false;
        if (F("nguoiLon")) F("nguoiLon").checked = !!nv.nguoiLon;
      }
    }
    {
      if (F("vaiBdsm")) nv.vaiBdsm = F("vaiBdsm").value;
      if (F("danhXung")) nv.danhXung = F("danhXung").value.trim();
      if (F("kinhNghiem")) nv.kinhNghiem = F("kinhNghiem").value;
      if (F("phongCach")) nv.phongCach = F("phongCach").value;
      if (F("khauVi")) nv.khauVi = F("khauVi").value.trim();
      if (F("gioiHan")) nv.gioiHan = F("gioiHan").value.trim();
      if (F("gioiHanCung")) nv.gioiHanCung = F("gioiHanCung").value.trim();
      if (F("luatRieng")) nv.luatRieng = F("luatRieng").value.trim();
      if (F("chamSocSau")) nv.chamSocSau = F("chamSocSau").value.trim();
      if (F("tinHieuRieng")) nv.tinHieuRieng = F("tinHieuRieng").value.trim();
      const soThichEl = bodyEl.querySelector('[data-f="soThich"]');
      if (soThichEl) nv.soThich = String(soThichEl.value || "").split(",").map((s) => s.trim()).filter(Boolean);
    }
    // ---- Chỉ từ đây mới chạm vào nhân vật thật ----
    const banTruoc = isNew2 ? null : JSON.parse(JSON.stringify(c));
    if (isNew2) story.nhanVats.push(nv);
    else {
      for (const k in c) delete c[k];
      Object.assign(c, JSON.parse(JSON.stringify(nv)));
    }
    if (!(await luuTruyen(story, "nhân vật"))) {
      ...
      if (isNew2) story.nhanVats = story.nhanVats.filter((x) => x !== nv);
      else {
        for (const k in c) delete c[k];
    ...
    toast(isNew2 ? "Đã thêm nhân vật " + c.ten : "Đã lưu nhân vật.");
    render();
    if (opts.onSaved) {
      try { opts.onSaved(c); } catch (e) { console.error(e); }
    }
    return true;
  }
